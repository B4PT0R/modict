...
class modict(dict, metaclass=modictMeta):
    ...
    Added features : 
        ...
        - support for recursive conversion of all nested dicts to modicts (including those found in nested mutable containers)
    ...
    def computed(cls, func=None, *, cache=False, deps=None):
        ...
        Usage as decorator (always with parentheses):
            ...
            @modict.computed(cache=True, deps=['sum_ab', 'c'])  # Dépend d'un autre computed !
            def final_result(self): return self.sum_ab + self.c
            
            @modict.computed(cache=True, deps=[])  # Never invalidate auto
    ...
    def fromkeys(cls, iterable, value=None):
        """Crée un modict depuis des clés avec validation."""
    ...
    def convert(cls, obj:Any, seen: Optional[Dict] = None, root: bool = True) -> 'modict':
        """
        Method used to convert dicts to modicts.
        Takes any obj as input.
        if obj is a dict : we upgrade to modict and continue to the next step
    ...
    def unconvert(cls, obj:Any, seen: Optional[Dict] = None) -> dict:
        """
        Method used to convert modicts to dicts recursively
        Takes any obj as input.
        if obj is a modict : we downgrade to dict and continue to next step
